...
runtime::Executor::Executor(runtime::Backend* backend)
    : m_backend{backend}
{
}
...
bool runtime::Executor::call(const vector<shared_ptr<runtime::Tensor>>& outputs,
...
bool runtime::Executor::call_with_validate(const vector<shared_ptr<runtime::Tensor>>& outputs,
                                           const vector<shared_ptr<runtime::Tensor>>& inputs)
{
    vector<runtime::Tensor*> out;
    vector<runtime::Tensor*> in;
...
bool runtime::Executor::validate_and_execute(const std::vector<runtime::Tensor*>& outputs,
...
void runtime::Executor::validate(const vector<runtime::Tensor*>& outputs,
...
const ngraph::ParameterVector& runtime::Executor::get_parameters() const
{
    return m_parameters;
}

const ngraph::ResultVector& runtime::Executor::get_results() const
{
    return m_results;
}

void runtime::Executor::set_parameters_and_results(const Function& func)
{
    m_parameters = func.get_parameters();
    m_results = func.get_results();
}

vector<runtime::PerformanceCounter> runtime::Executor::get_performance_data() const
{
    return vector<PerformanceCounter>();
}

runtime::Backend* runtime::Executor::get_backend()
{
    return m_backend;
}
